...
func TestGetFollowerList(t *testing.T) {
	...
	a := []model.Follow{{Id: 0, FollowId: 1, FollowerId: 1, IsDeleted: 0x0}}
...
func TestGetFollowInfoById(t *testing.T) {
	...
	a := model.FollowInfo{FollowCount: 1, FollowerCount: 1, IsFollow: true}
	assert.Equal(t, a, isPass, "测试获取关注列表失败")
}
...
func TestGetLastMessage(t *testing.T) {
	...
	a := model.Message{FromUserId: 1, ToUserId: 1}
...
func TestGetUserFollowInfo(t *testing.T) {
	...
	a := model.FollowInfo{FollowCount: 0, FollowerCount: 0, IsFollow: false}
